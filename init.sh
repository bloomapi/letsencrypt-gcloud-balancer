...
./lego_linux_amd64 $USE_STAGING_SERVER -m $LETSENCRYPT_EMAIL -dns gcloud $DOMAINS_LIST -a run
...
OLD_CERT_ID=$(./google-cloud-sdk/bin/gcloud -q compute target-https-proxies list --filter "name=rancher-chat-lb-target-proxy" | sed -n 2p | awk '{print $2}')
